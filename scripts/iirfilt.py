...
# apply
xq = qfrac(stimuli([500, 4500], fs, 1024), nb-1)
...
nb_new = nb
while not np.array_equal(qfrac(sos_new, nb_new-1), sos_new): 
   nb_new +=1
print(nb_new)
print(qfrac(sos_new, nb_new-1))
print(sos_new*(pow(2, nb_new-1)))

yq = qfrac(signal.lfilter(sos_new[0,:4], sos_new[0,4:], xq), nb-1)
np.savetxt('results2', yq*(pow(2, nb-1)), fmt="%d")
